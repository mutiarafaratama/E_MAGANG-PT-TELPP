...
func NewNotifikasiService(repo *repository.NotifikasiRepository, hub *ws.Hub, fcm *FCMService) *NotifikasiService {
        return &NotifikasiService{repo: repo, hub: hub, fcm: fcm}
}

// Kirim simpan notifikasi ke DB dan push ke WebSocket + FCM jika user sedang offline.
func (s *NotifikasiService) Kirim(ctx context.Context, userID uuid.UUID, judul, pesan, tipe string, refID *uuid.UUID) {
        ...
        // FCM push — pakai context.Background() agar tidak ikut cancel saat HTTP handler selesai
        go func() {
...
// SaveFCMToken — simpan/update FCM token di tabel users
func (s *NotifikasiService) SaveFCMToken(ctx context.Context, userID uuid.UUID, token string) error {
